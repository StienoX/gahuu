...
processAI ai@(MkAI AI1 aiPos hitbox rect) speed player hitboxes | (flipTuple aiPos) > (intToFloat screenHeight,0)                                   = Nothing
                                                                | posSub aiPos (pos player) < (0,0) && aiCollided (moveAI ai posSub speed) hitboxes = Just (moveAI ai posSub speed)
                                                                | aiCollided (moveAI ai posAdd speed) hitboxes                                      = Just (moveAI ai posAdd speed)
                                                                | otherwise                                                                         = Just (ai)
...
processAI ai@(MkAI AI3 aiPos hitbox rect) speed player hitboxes | (flipTuple aiPos) > (intToFloat screenHeight,0)                                   = Nothing
                                                                | posSub aiPos (pos player) < (0,0) && aiCollided (moveAI ai posAdd speed) hitboxes = Just (moveAI ai posAdd speed)
                                                                | aiCollided (moveAI ai posSub speed) hitboxes                                      = Just (moveAI ai posSub speed)
                                                                | otherwise                                                                         = Just (ai)
